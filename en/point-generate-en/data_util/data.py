...
def read_example_convert_to_feature(example_path_name,article_len,abstract_len,vocab,index,point = True):
    ...
    article_indexes = [vocab.word_2_idx(word) for word in article]
    ...
    # 截断，限制摘要的长度
    abstract = abstract[:abstract_len+1]
    ...
    abstract_indexes = [vocab.word_2_idx(word) for word in abstract]

    decoder_input = abstract_indexes[:-1]
...
def get_features(token_dir,feature_dir,vocab,args,data_set = "train",example_num = 1024*8):
    ...
    sample_num = len(token_file_list)
