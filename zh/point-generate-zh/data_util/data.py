...
def article_word_to_idx_with_oov(content_list,vocab):
    indexes = []
    oovs = []
...
def abstract_target_idx_with_oov(title_list, vocab , oovs):
    target_with_oov = []
...
def from_sample_covert_feature(vocab,news_id,title,content,index,content_len,title_len,point = True):
    ...
    if point:
        encoder_input_with_oov,oovs = article_word_to_idx_with_oov(content,vocab)
        decoder_target_with_oov = abstract_target_idx_with_oov(title,vocab,oovs)
